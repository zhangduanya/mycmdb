...
class ArgvHandler(object):
    ...
    def parse_args(self):
        ...
        if len(self.args) > 1 and hasattr(self, self.args[1]):
            ...
            self.help_msg()     ##如果执行程序没有带参数就会提示如下信息：help_msg()

    @staticmethod       #静态方法 类或实例均可调用,静态方法函数里不传入self，这样如上self.help_msg()就可以调用了
    def help_msg():
        """
        帮助说明
        :return:
    ...
    def report_data():
        ...
        # 记录发送日志
        with open(settings.PATH, 'ab') as f:        ##a追加,b二进制文件
